...
class ContratoController extends Controller
{
    ...
    public function contratos(request $request){
        ...
        return view('admin.contratos', compact('persona', 'contratos'));
    ...
    public function rechazarContrato($id_contrato){
        ...
        if (!$succes) {
            return redirect()->route('admin.contratos')->with('error', 'Error al rechazar el contrato.');
        }

        return redirect()->route('admin.contratos')->with('success', 'Contrato rechazado correctamente.');
    }
    ...
    public function generarBoleta($id_contrato){
        ...
        $contrato = $contrato[0];
        return view('admin.generar_boleta', compact('persona', 'contrato'));
    }
    ...
    public function aceptarContrato(Request $request){
        ...
        if (!$succes) {
            return redirect()->route('admin.contratos')->with('error', 'Error al aceptar el contrato.');
        }
        $succes = BoletaPago::generarBoleta($id_contrato, $ruta_boleta);
        if (!$succes) {
            return redirect()->route('admin.contratos')->with('error', 'Error al generar la boleta.');
        }
        return redirect()->route('admin.contratos')->with('success', 'Boleta generada correctamente.');
    }
